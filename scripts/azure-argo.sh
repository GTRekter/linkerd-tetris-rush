...
echo "Applying Argo CD manifests (initial — with placeholder URLs)..."
...
# The QR code and /go redirect depend on externalUrl and dashboardUrl,
# which point to LoadBalancer IPs that aren't known until Argo deploys
# the Services. Wait for the IPs, update the Argo manifests, and
# re-apply so the Applications carry the correct values permanently.
...
dashboard_url="https://${dash_ip}:8090"
echo "Dashboard URL: ${dashboard_url}"

# Inject dashboardUrl and update externalUrl in the platform Application
DASH_URL="$dashboard_url" perl -i -pe '
    if (/externalUrl: "http:\/\/platform\.localhost:\d+"/) {
        $_ = qq{        externalUrl: "$ENV{DASH_URL}"\n        dashboardUrl: "$ENV{DASH_URL}"\n};
    }
' "$argo_temp/tetris.yaml"
...
for context in "${gameplay_clusters[@]}"; do
    ...
    game_url="http://${game_ip}"
    echo "  $context → ${game_url}"
    CONTEXT="$context" GAME_URL="$game_url" perl -i -pe '
        s/externalUrl: "http:\/\/\Q$ENV{CONTEXT}\E\.localhost:\d+"/externalUrl: "$ENV{GAME_URL}"/
    ' "$argo_temp/tetris.yaml"
done

echo "Re-applying Argo CD manifests with real LoadBalancer URLs..."
kubectl --context=platform apply -f "$argo_temp/"
rm -rf "$argo_temp"
